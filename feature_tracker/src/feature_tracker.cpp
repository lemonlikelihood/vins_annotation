...
void FeatureTracker::setMask()
{
    ...
    {   
        ...
        if (mask.at<uchar>(it.second.first) == 255) //重复点就不管了
        {
...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
    ...
    if (EQUALIZE)
    {
        ...
        cv::Ptr<cv::CLAHE> clahe = cv::createCLAHE(3.0, cv::Size(8, 8)); //8X8的直方图自适应
        TicToc t_c;
    ...
    if (cur_pts.size() > 0)
    {
        ...
        reduceVector(prev_pts, status);//为什么之前帧的也得处理
    ...
    //通过基本矩阵剔除outliers
    if (PUB_THIS_FRAME) //如果不需要public，就不会强行给这一帧找点了
    {   
        //通过基本矩阵剔除outliers
        rejectWithF();
        ...
        if (n_max_cnt > 0)
        {
            ...
            */
            ...
            cv::goodFeaturesToTrack(forw_img, n_pts, MAX_CNT - forw_pts.size(), 0.01, MIN_DIST, mask); //为了在没有点的区域，再生成一些角点
        }

        //每次追踪后，下一帧的feature来自两部分：1.光流get(对于失败的点，我们会将前帧，当前帧，和后一帧的结果都clean) 2.剩余部分，直接内部角点获取
        else
            n_pts.clear();
