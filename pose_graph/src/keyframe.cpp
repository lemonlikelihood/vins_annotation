...
{
	//注意，这里我们传入的_vio_T_w_i和_vio_R_w_i来自于vins_estimator中，传入的就是Twbj
	...
	sequence = _sequence;
	computeWindowBRIEFPoint(); //基于传入的point_2d_uv计算角点，生成描述子，放到window中
	computeBRIEFPoint(); //直接生成新的角点，两种方式，一致直接基于fast提角点，另一种提500个角点，再基于提的点生成描述子
	if(!DEBUG_IMAGE)
		image.release();
}
...
void KeyFrame::computeBRIEFPoint()
{
	...
	if(1)
		cv::FAST(image, keypoints, fast_th, true); //两种思路提点，一种fast提点，只要圈内的灰度差的足够多，就行
	else
	{
...
//windows_berif是我们对当前帧的提取，将win中的描述子和找到的回环帧的描述子全体进行比较，筛选出windows比较好的点
                                ...
                                const std::vector<cv::KeyPoint> &keypoints_old_norm)
{
    for(int i = 0; i < (int)window_brief_descriptors.size(); i++)
    {
		...
		//将window里的所有点，都去和descriptor全体进行比较，找到最好的match
                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
    ...
    Matrix3d R_inital;
    Vector3d P_inital;
	//qic和tic都是camera和IMU之间的，进而我们可以将Twbj转移到Twcj中，因为在pnp的solve中，是世界点和相机中的归一化坐标进行比较优化
    ...
    /*
	设当前帧维第j帧，回环参考帧维第i帧，因为我们取得是当前帧的三维点，所以求出的R，t都是从bj->ci的，注意此时三维点是在body系下的
	进而我们进行了转化，求出了ci->w的R，t,注意和vins中的对比，因为在vins的优化过程中，我们默认了参考帧是世界坐标系，所以实际上求出的T是两帧之间的
	但是这里的三维点是真正的world下的点，所以求出的是参考回环帧到世界的T
	*/
    cv::Rodrigues(rvec, r);
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	//将当前帧和挑选出来的回环帧进行比较，找到当前帧中比较好的角点
	...
	*/
	...
	double relative_yaw;
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM) //如果匹配的个数超过阈值，则进行筛选
	{
		status.clear();
		//利用回环帧的2d点和当前帧的3d点
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
		//此时我们已经从pnp中获得了参考帧到世界坐标系的平移和旋转
		//从而求出了两帧之间的T，放入loop_info中
