...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    Vector3d vio_P_cur;
    Matrix3d vio_R_cur;
    if (sequence_cnt != cur_kf->sequence)//一般不会发生，只有当当前序列结束时，即我们重新开始定义一个base frame、
    {
        sequence_cnt++;
        sequence_loop.push_back(0);
        w_t_vio = Eigen::Vector3d(0, 0, 0);
    ...
    {
        TicToc tmp_t;
        //我们基于等DOW2建立了一个数据库，每当detectLoop时，就会将当前帧的描述子存入，扩大数据库
        //然后基于描述子距离找到最match的回环帧
        loop_index = detectLoop(cur_kf, cur_kf->index); //返回回环检测的候选帧id
    }
    else
    {
	...
	if (loop_index != -1)
	{
        ...
        //我们会将所有的回环帧(之前的关键帧，在node代码中每当一定间隔就认为来了一个关键帧)放入keyframe_list，供查询
        ...
        if (cur_kf->findConnection(old_kf))
        {
            ...
            //根据两者的相互关系重新估算当前帧的位姿
            /*
            注意这里两帧的世界坐标系不一样，即Tw1bi和Tw2bj
            这里求出了Tw2bi
            */
            w_P_cur = w_R_old * relative_t + w_P_old;
            w_R_cur = w_R_old * relative_q;
            
            //计算我们估算的的位姿和通过相互关系计算的位姿之间的偏移
            //求出了Tw1w2
            double shift_yaw;
            Matrix3d shift_r;
            Vector3d shift_t; 
            ...
            //sequence_loop在最开始时就加入了，默认设为的是0
            //进而求出了Tw1bj,将当前帧转移到参考帧的世界坐标系中
            ...
            {  
                ...
                sequence_loop[cur_kf->sequence] = 1; //0和1来区分
            }
            m_optimize_buf.lock();
            optimize_buf.push(cur_kf->index);  //把当前帧放入优化对列
...
void PoseGraph::loadKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    if (loop_index != -1)
    {
        printf(" %d detect loop with %d \n", cur_kf->index, loop_index);
        KeyFrame* old_kf = getKeyFrame(loop_index);
        if (cur_kf->findConnection(old_kf)) //1.检测是否满足回环条件，如果满足就会将回环帧的索引和相对位姿都存入loop_index和loop_info
        {
            if (earliest_loop_index > loop_index || earliest_loop_index == -1)
                earliest_loop_index = loop_index;
            m_optimize_buf.lock();
