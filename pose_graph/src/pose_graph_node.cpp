...
int SKIP_CNT; //运行process中循环的间隔
int skip_cnt = 0;
bool load_flag = 0;
bool start_flag = 0;
double SKIP_DIS = 0; //判断是否构建关键帧的距离标准

int VISUALIZATION_SHIFT_X;  //可视化界面中图像x轴和y轴的偏移，一般设为0
int VISUALIZATION_SHIFT_Y;
int ROW;
int COL;
int DEBUG_IMAGE;
int VISUALIZE_IMU_FORWARD;
int LOOP_CLOSURE;  //如果等于1，则进行回环检测
    ...
    while (true)
    {
        ...
        if (pose_msg != NULL)
        {
            ...
            if((T - last_t).norm() > SKIP_DIS)
            {
                ...
                m_process.lock();
                start_flag = 1;
                posegraph.addKeyFrame(keyframe, 1);//1表示进行回环检测
                m_process.unlock();
                frame_index++;
                last_t = T;
            }
...
int main(int argc, char **argv)
{
    ...
    //订阅各个topic，执行各自的回调函数
    ros::Subscriber sub_imu_forward = n.subscribe("/vins_estimator/imu_propagate", 2000, imu_forward_callback); //得到的是当前输入的imu的信息
    ros::Subscriber sub_vio = n.subscribe("/vins_estimator/odometry", 2000, vio_callback); //全体的P,Q,V
    ros::Subscriber sub_image = n.subscribe(IMAGE_TOPIC, 2000, image_callback); //所有IMU的P，Q，V
