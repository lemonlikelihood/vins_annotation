...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_odometry = n.advertise<nav_msgs::Odometry>("odometry", 1000);     //发布的Ps[w,bk],Rs[w,bk],Vs[w,bk]
    pub_point_cloud = n.advertise<sensor_msgs::PointCloud>("point_cloud", 1000);  //所有track次数超过两次的世界点
    pub_margin_cloud = n.advertise<sensor_msgs::PointCloud>("history_cloud", 1000); //start_frame为0的会被marge的点
    pub_key_poses = n.advertise<visualization_msgs::Marker>("key_poses", 1000);  //发布的是窗口内所有帧的平移信息，即Ps[w,bk]
    pub_camera_pose = n.advertise<nav_msgs::Odometry>("camera_pose", 1000);   //发布的信息是位姿T[w,ck]，此处忽略了初始化的那几帧，将信息从bk系转移到了ck系
    pub_camera_pose_visual = n.advertise<visualization_msgs::MarkerArray>("camera_pose_visual", 1000); //发布的信息就是R和t,和camera_pose一致
    pub_keyframe_pose = n.advertise<nav_msgs::Odometry>("keyframe_pose", 1000); //非marge的都是keyframe
...
void pubCameraPose(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        int i = idx2;  //最开始初始化的那几帧，我们是不care的，所以从window_size处开始发送比较合理
       
       
        //Ps[i]就是t[w,bk]，Rs就是R[w,bk]
        //tic[0]表示的是t[b,c],表示的是camera到IMU坐标系的平移(即到b0的平移) 
        //t从另外一个角度可以解释为camera坐标系的原点在b0系下的坐标，所以此时P得到的就是camera在w系下的坐标
